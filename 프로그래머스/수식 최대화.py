...
sOperator = set(operator)

print(sOperator)

priority = list(permutations(sOperator, len(sOperator)))

print(priority)


def calculate(pExp, pOperator, pPriority):
    if len(pOperator) == 0:
        return pExp

    exist = pPriority[0] in pOperator

    while exist:
        index = pOperator.index(pPriority[0])
        temp = int(pExp.pop(index))
        if pOperator[index] == '+':
            temp += int(pExp.pop(index))
        elif pOperator[index] == '-':
            temp -= int(pExp.pop(index))
        else:
            temp *= int(pExp.pop(index))
        pOperator.pop(index)
        pExp.insert(index, temp)
        exist = pPriority[0] in pOperator

    pPriority.pop(0)

    pExp = calculate(pExp, pOperator, pPriority)
...
for i in priority:
    cal.extend(calculate(exp.copy(), operator.copy(), list(i).copy()))
print(cal)
