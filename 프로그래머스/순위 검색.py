...
# backend 직군을 선택했고 junior 경력이면서 소울푸드로 pizza를 선택한 지원자 중 코딩테스트 점수를 100점 이상 받은 지원자는 1명 입니다.
# "python and frontend and senior and chicken 200" : python으로 코딩테스트를 봤으며,
# frontend 직군을 선택했고, senior 경력이면서 소울 푸드로 chicken을 선택한 지원자 중 코딩테스트 점수를 200점 이상 받은 지원자는 1명 입니다.
# "cpp and - and senior and pizza 250" : cpp로 코딩테스트를 봤으며,
# senior 경력이면서 소울푸드로 pizza를 선택한 지원자 중 코딩테스트 점수를 250점 이상 받은 지원자는 1명 입니다.
# "- and backend and senior and - 150" : backend 직군을 선택했고,
# senior 경력인 지원자 중 코딩테스트 점수를 150점 이상 받은 지원자는 1명 입니다.
# "- and - and - and chicken 100" : 소울푸드로 chicken을 선택한 지원자 중 코딩테스트 점수를 100점 이상을 받은 지원자는 2명 입니다.
# "- and - and - and - 150" : 코딩테스트 점수를 150점 이상 받은 지원자는 4명 입니다.

# 아래에 주석되어있는 부분으로 짰으나 효율성 테스트에서 탈락
# info가 주어질 때 해당되는 모든 경우의 수를 만들어 dictionary로 저장 후
# query가 주어질 때 dictionary를 찾아 점수 list를 이진 탐색
# 구글 서치 후 찾았으나 실제로 응용하려면 공부가 더 필요함!
...
for q in query:
    ...
    query_list.extend(query_list.pop().split())

    key = ''.join(query_list[:-1])
    value = int(query_list[-1])

    idx = bisect_left(dic[key], value)

    #print(len(dic[key]) - idx)
...
#             if k == len(query_list) - 1:
#                 if int(j[k]) < int(query_list[k]):
#                     flag = 0
#                     break
#             else:
#                 if not query_list[k] == '-':
