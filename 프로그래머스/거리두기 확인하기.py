...
for i in places:
    ...
    for j in range(0, len(p)):
        for k in range(j + 1, len(p)):
            distance = abs(p[j][0] - p[k][0]) + abs(p[j][1] - p[k][1])
            #print(distance)
            if distance <= 2:
                x_flag = 0
                y_flag = 0
                if p[j][0] == p[k][0]:
                    ...
                    for a in range(p[j][0] - 1, p[k][0] - 1, -1):
                        if i[a][p[j][1]] == 'X':
                            x_flag = 1
                            #print("check %d %d" %(a,p[j][0]))
                else:
                    for a in range(p[j][0] + 1, p[k][0] + 1):
                        if i[a][p[j][1]] == 'X':
                            x_flag = 1
                            #print("check %d %d" % (a, p[j][0]))
                if p[j][1] == p[k][1]:
                    ...
                    for a in range(p[j][1] - 1, p[k][1] - 1, -1):
                        if i[p[j][0]][a] == 'X':
                            y_flag = 1
                            #print("check %d %d" % (p[j][1], a))
                else:
                    for a in range(p[j][1] + 1, p[k][1] + 1):
                        if i[p[j][0]][a] == 'X':
                            y_flag = 1
                            #print("check %d %d" % (p[j][1], a))
                if not (x_flag == 1 and y_flag == 1):
                    flag = 1
