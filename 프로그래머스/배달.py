...
# DFS를 이용하여 모든 경로를 탐색하는 방법 선택 시 시간 초과 발생 (주석 처리)
# 다익스트라 알고리즘을 이용해 모든 노드 최단 경로를 구하여 처리
...
#
#
# def dfs(result, time, node, dic, target, visited):
#     if visited[node]:
#         return result
#
#     if target == node:
...
#
#     for r in road:
#         ret = dfs(result, time + r[1], r[0], dic, target, visited)
#         if result == 0 or result > ret:
#             result = ret
