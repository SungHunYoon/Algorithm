...
for i in range(1, cols + 1):
    combi.extend(combinations(range(cols), i))

#print(combi)

unique = []
for c in combi:
    tmp = [tuple(r[key] for key in c) for r in relation]  # 조합 튜플을 만들어 key list로 만든다.
    if len(set(tmp)) == rows:  # set으로 중복 제거 후 rows 랑 같으면 unique 만족
        unique.append(c)

answer = set(unique)  # 최소성 검출 시 set 멤버 삭제를 위한 변환
...
for i in range(len(unique)):
    for j in range(i + 1, len(unique)):
        ...
        # print('------------------------------')
        if len(unique[i]) == len(set(unique[i]) & set(unique[j])):  # 교집합 갯수가 unique와 동일하면 최소성을 만족하지 않음
